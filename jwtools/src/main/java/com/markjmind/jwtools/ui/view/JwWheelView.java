...
import com.markjmind.uni.ui.view.textview.JwTextViewFonts;
...
public class JwWheelView extends LinearLayout  implements OnTouchListener{
	...
	public void postInit(){
		...
		if(isHorizontal){
			mainLyt.setOrientation(LinearLayout.VERTICAL);
			LayoutParams lp2 = new LayoutParams(measure*(maxCount-1)+getWidth(), LayoutParams.MATCH_PARENT);
			...
			mainLyt.setOrientation(LinearLayout.HORIZONTAL);
			LayoutParams lp2 = new LayoutParams(LayoutParams.MATCH_PARENT, measure*(maxCount-1)+getHeight());
			mainLyt.setLayoutParams(lp2);
			for(int i=0;i<lines.size();i++){
				LineInfo info = lines.get(i);
				lineLayout = info.layout;
...
//			else{mc.setBackgroundColor(Color.GREEN);}
	...
	class MeasureCellLyt extends LinearLayout{
		public void setMeasure(int measure, View child){
			if(isHorizontal){
				LayoutParams lp = new LayoutParams(measure, LayoutParams.WRAP_CONTENT);
				this.setLayoutParams(lp);
				setOrientation(LinearLayout.HORIZONTAL);
				setGravity(Gravity.CENTER_HORIZONTAL);
			}else{
				LayoutParams lp = new LayoutParams(LayoutParams.WRAP_CONTENT,measure);
				this.setLayoutParams(lp);
	...
	public class DefaultCell extends LinearLayout{
		...
		public DefaultCell(Context context,int lineWidth, int lineHeight){
			...
			if(isHorizontal){
				setOrientation(LinearLayout.HORIZONTAL);
				LayoutParams lp2 = new LayoutParams((int)(lineWidth*density),(int)(lineHeight*density));
				setLayoutParams(lp2);
			}else{
				setOrientation(LinearLayout.VERTICAL);
				LayoutParams lp2 = new LayoutParams((int)(lineHeight*density), (int)(lineWidth*density));
				setLayoutParams(lp2);
			}
		}
	}
